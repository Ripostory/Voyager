...
vec4 sampleNearest(vec2 base, float distance);
float sampleAO(vec2 base, float distance);
...
{	
    ...
    float metallic = 0; //disables metallic if theres warp
    ...
    vec3 finalLight = (finalDiff * finalColor / 3.1415 + specular) * lightColor * lightAngle * AO;
	...
	//gl_FragColor = vec4(perlinOutput, 1.0f);
}
...
//samples four nearest corners
vec4 sampleNearest(vec2 base, float distance)
{
	float north = distort(vec2(base.x, base.y+distance), inpSeed);
	float south = distort(vec2(base.x, base.y-distance), inpSeed);
	float east = distort(vec2(base.x+distance, base.y), inpSeed);
	float west = distort(vec2(base.x-distance, base.y), inpSeed);
	return vec4(north, south, east, west);
}

float sampleAO(vec2 base, float distance)
{
	vec4 side = sampleNearest(base, distance);
    float me = distort(base, inpSeed);

    //get average strength of AO
    float avgStr = side.x+side.y+side.z+side.w;
    avgStr /= 4.0;
    return clamp(1 + avgStr - me, 0, 1);
}
